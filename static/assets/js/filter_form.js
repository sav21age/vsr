(function () {
    ...
    const dff = select('#decFilterForm');
    if (dff) {
        const urlSearchParams = new URLSearchParams(window.location.search);
        const showFilters = urlSearchParams.get('show_filters');
        if (showFilters == 'yes') {
            const cdff = select('#collapseDecFilterForm');
            bootstrap.Collapse.getOrCreateInstance(cdff).show();
        }
        ...
        const send = function (e) {
            ...
            if (!height_from.classList.contains('is-invalid')) {
                xhr.open("POST", '/catalog/deciduous/filter-form/', true);
                xhr.setRequestHeader("X-REQUESTED-WITH", "XMLHttpRequest");
                xhr.onreadystatechange = function () {
    ...
    const pff = select('#perFilterForm');
    if (pff) {
        const urlSearchParams = new URLSearchParams(window.location.search);
        const showFilters = urlSearchParams.get('show_filters');
        if (showFilters == 'yes') {
            const cpff = select('#collapsePerFilterForm');
            bootstrap.Collapse.getOrCreateInstance(cpff).show();
        }
        ...
        const send = function (e) {
            ...
            let planting_year = select("select[name=planting_year]");
            if (planting_year.value) body['planting_year'] = planting_year.value
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
                    ...
                    if (Object.keys(res).length !== 0) {
                        ...
                        if (res['planting_year'] && e.target.name != 'planting_year') {
                            selectField(planting_year, res['planting_year'])
                        }
                    }
                    select('.overlay').classList.remove('d-block');
                }
